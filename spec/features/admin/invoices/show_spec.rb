...
RSpec.describe "Admin Invoices Index Page" do
  before :each do
    ...
    @ii_11 = InvoiceItem.create!(invoice_id: @invoice_1.id, item_id: @item_8.id, quantity: 13, unit_price: 6, status: 1)
  ...
  it "displays the id, status and created_at" do
  ...
  it "has status as a select field that updates the invoices status" do
    visit admin_invoice_path(@invoice_1)
  ...
  it "shows both the subtotal and grand total(discount applied) on invoice with no coupon applied" do
    ...
    within("#totals") do
      expect(page).to have_content("Subtotal Revenue:")
      ...
      expect(page).to have_content("Grand Total Revenue:")
  ...
  it "displays the customers name and shipping address" do
    visit admin_invoice_path(@invoice_1)

    within("#customer-info") do
      expect(page).to have_content("#{@customer_1.first_name} #{@customer_1.last_name}")
      expect(page).to have_content(@customer_1.address)
      expect(page).to have_content("#{@customer_1.city}, #{@customer_1.state} #{@customer_1.zip}")
  ...
  it "displays all the items on the invoice" do
    ...
    within("#items-on-invoice") do
      ...
      expect(page).to have_content(@ii_11.status)
